...
def insert_logs(client: MilvusClient, emb_dim):
    logs = [
        ...
        # 插 1 个词（slop=1 命中）
        "fatal: connection fast reset by peer",

        # 插 2 个词（slop=2 命中）
        "warning: connection was unexpectedly reset by peer",

        # 插 3 个词（slop=3 才能匹配）
        "notice: connection was very unexpectedly reset by peer",

        # 倒序（slop=3）
        "peer reset the connection",

        # 相似但不属于 reset（TEXT_MATCH 会误命中）
        ...
        # 完全无关
        "authentication token expired for peer"
    ]
